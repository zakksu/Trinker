...
def export_sessions_csv(
    ...
    fieldnames = [
        "id", "build_order_id", "date", "duration_sec", "result",
        "feudal_time_sec", "castle_time_sec", "imperial_time_sec",
        "final_pop", "food_at_feudal", "wood_at_feudal", "gold_at_feudal",
        "stone_at_feudal", "accuracy_pct", "notes", "replay_path", "created_at",
    ]
...
def export_build_orders_csv(path: Optional[str] = None) -> str:
    ...
    fieldnames = ["id", "name", "civ", "strategy", "difficulty", "author",
                  "source_url", "step_count", "is_favorite", "created_at"]
    with out.open("w", newline="", encoding="utf-8") as fh:
        w = csv.DictWriter(fh, fieldnames=fieldnames)
        w.writeheader()
        for bo in bos:
            w.writerow({
                "id": bo.id, "name": bo.name, "civ": bo.civ,
                "strategy": bo.strategy, "difficulty": bo.difficulty,
                "author": bo.author, "source_url": bo.source_url,
                "step_count": bo.step_count, "is_favorite": bo.is_favorite,
